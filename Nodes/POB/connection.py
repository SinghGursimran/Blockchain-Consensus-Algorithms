...
class Peer(Node):
    ...
    def node_message(self, node, data):
        ...
        if data['type'] == 'new_txn':
            ...
            #burning some coins - symbolic of what the user would have burnt before recieving the mining control. 
            #In practice, This is generally some other cryptocurrency.
            txn = Transaction(self.id, None, self.blockchain.burn)
            self.blockchain.createTransaction(txn)
            self.broadcast(txn, data['obj']['stage'], type='new_txn')
            self.broadcast(None, data['obj']['stage'], type = 'ack')
