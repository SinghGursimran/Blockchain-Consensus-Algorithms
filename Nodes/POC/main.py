...
def create_local_nodes(n):
	...
	total = shutil.disk_usage("/home/")[2]
	weights = []
	for i in range(n): weights.append(random.randint(1,100))

	for i in range(n):
		port = ports[i]
		capacity = (total*weights[i])//sum(weights)
		node = Peer(host, port, capacity)
		node.start()
		nodes.append(node)
	return nodes
