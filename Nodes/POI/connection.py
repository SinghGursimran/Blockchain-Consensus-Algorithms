...
class Peer(Node):
    ...
    def max_transactions(self):
        ...
        for block in self.blockchain.chain: 
            for transaction in block.transactions:
                if(type(transaction) == dict): continue
                if(transaction.fromAddress != " " and transaction.fromAddress != None): d[transaction.fromAddress] = d.get(transaction.fromAddress, 0) + 1
        if(d == {}): return self.id
        return max(d.items(), key=operator.itemgetter(1))[0]
        if("ugster" in res): return res
        else: return self.id
    ...
    def broadcast(self, obj, stage, type):
        ...
        if type == 'new_txn':
        	...
        	payload['imp'] = self.max_transactions()
        payload = json.dumps(payload)

        for node in self.nodes_outbound:
            if(type == "ack" or type == "imp"): 
    ...
    def node_message(self, node, data):
        ...
        if data['type'] == 'new_txn':
        	...
        	self.broadcast(None, data['obj']['stage'], type = 'imp') 
